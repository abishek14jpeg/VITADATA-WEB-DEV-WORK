...
export default function AdminLoginPage({ onLogin, onSignUp, onHome }) {
    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");

    const handleSubmit = (e) => {
        e.preventDefault();
        // Check simple validation or simulate login success
        if (onLogin) onLogin();
    };

    return (
        <div className="min-h-screen bg-[#F5F0EB] flex flex-col items-center justify-center font-sans text-[#3D2010] relative overflow-hidden">
            {/* Symmetrical Floating Blobs specific to Admin portal */}
            <div className="absolute top-0 left-0 w-96 h-96 bg-[rgba(217,119,87,0.1)] rounded-full blur-3xl z-0 pointer-events-none" />
            <div className="absolute bottom-0 right-0 w-96 h-96 bg-[rgba(255,204,172,0.15)] rounded-full blur-3xl z-0 pointer-events-none" />
            ...
            <div className="bg-white rounded-[16px] shadow-[0_4px_24px_rgba(217,119,87,0.08)] w-full max-w-[320px] sm:max-w-[360px] p-5 sm:p-7 flex flex-col items-center mx-4 border border-[rgba(255,204,172,0.3)] relative z-10 animate-fade-up">
                ...
                {/* Heading */}
                <h1 className="text-[20px] sm:text-[22px] font-bold text-[#3D2010] mb-1.5">Admin Sign In</h1>
                <p className="text-[#6B7280] text-[13px] sm:text-[14px] mb-4 text-center">Manage hospital operations and resources</p>
                ...
                <form className="w-full" onSubmit={handleSubmit}>
                    <div className="mb-3.5">
                        ...
                        <input
                            type="email"
                            placeholder="admin@vitadata.com"
                ...
                {/* Footer Link */}
                <p className="text-[#9C8276] text-[14px] text-center">
                    New admin branch?{' '}
